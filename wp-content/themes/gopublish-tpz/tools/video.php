			...
			<div class="widgetwrap">
						...
						if ( $video ) {
							...
							if (
								...
								<?php if ( $instance['show-writer'] == "on" ) {
									...
									if ( $videographer ) {
										?>
										<p class="photocredit">Video Credit: <?php echo $videographer; ?></p>
									<?php }
								}
								if ( $instance['show-headline'] == "on" ) {
									$headlinesize   = $instance['headline-size'];
									$headlineheight = floor( $headlinesize * 1.5 ); ?>
									<a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title(); ?>">
										<h3 class="homeheadline" style="font-size:<?php echo $headlinesize; ?>px; line-height:<?php echo $headlineheight; ?>px;margin-top:0px"><?php the_title(); ?></h3>
									</a>
								<?php }
								if ( $instance['show-teaser'] == "on" ) {
								...
								if (
									...
									<p class="datetime">
										...
										edit_post_link( 'Edit this story', ' &bull; ', '' ); ?>
									</p>
								<?php } ?>
							</div>
						...
						<div<?php if ( $instance['remove-padding'] == 'on' ) { ?> style="padding-left:10px;padding-right:10px"<?php } ?>>
							<?php if ( ( $videocount == $videodivider ) && ( $instance['headline-header'] == 'on' ) ) { ?>
								<a href="/<?php echo $categoryslug; ?>">
									<p class="sectionhead" style="font-size:16px;padding-top:15px;margin-bottom:7px;font-weight:bold;">Recent Videos</p>
								</a>
							<?php } else { ?>
							...
							<p>
								<a class="homeheadline" href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
							</p>
							<?php if ( $instance['teaser-date'] == 'on' ) { ?>
								<p>
									<?php the_time( ' F j, Y' ); ?>
								</p>
							<?php } ?>
				...
				if ( $instance['view-all'] == on ) { ?>
					<?php if ( $instance['dividing-line'] == on ) { ?>
						<div class="storybottom" <?php if ( $instance['remove-padding'] ) { ?>style="margin-left:10px;margin-right:10px;"<?php } ?>></div><?php } else { ?>
						<div class="storybottomnoline"></div><?php } ?>
					<a href="<?php echo cat_id_to_slug( $instance['category'] ); ?>">
						<p class="viewall" <?php if ($instance['remove-padding']) { ?>style="margin-left:10px;margin-right:10px;padding-bottom:5px"<?php } ?>>View All
							<span class="arrows"> &raquo;</span></p></a>
				<?php } ?>
		...
		<div style="float:left;width:230px;margin-right:20px;border-right:1px solid #aaaaaa;padding-right:10px;">
			...
			<p>
				...
				<input class="checkbox" type="checkbox" <?php if ( $instance['show-headline'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'show-headline' ); ?>" name="<?php echo $this->get_field_name( 'show-headline' ); ?>" />
			...
			<p>
				<input class="checkbox" type="checkbox" <?php if ( $instance['show-teaser'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'show-teaser' ); ?>" name="<?php echo $this->get_field_name( 'show-teaser' ); ?>" />
			...
			<p>
				<input class="checkbox" type="checkbox" <?php if ( $instance['show-writer'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'show-writer' ); ?>" name="<?php echo $this->get_field_name( 'show-writer' ); ?>" />
				...
				<input class="checkbox" type="checkbox" <?php if ( $instance['show-date'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'show-date' ); ?>" name="<?php echo $this->get_field_name( 'show-date' ); ?>" />
				...
				<input class="checkbox" type="checkbox" <?php if ( $instance['show-comments'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'show-comments' ); ?>" name="<?php echo $this->get_field_name( 'show-comments' ); ?>" />
			...
			<p>
				<input class="checkbox" type="checkbox" <?php if ( $instance['remove-padding'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'remove-padding' ); ?>" name="<?php echo $this->get_field_name( 'remove-padding' ); ?>" />
				...
				<input class="checkbox" type="checkbox" <?php if ( $instance['dividing-line'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'dividing-line' ); ?>" name="<?php echo $this->get_field_name( 'dividing-line' ); ?>" />
			...
			<p>
				<input class="checkbox" type="checkbox" <?php if ( $instance['headline-header'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'headline-header' ); ?>" name="<?php echo $this->get_field_name( 'headline-header' ); ?>" />
				...
				<input class="checkbox" type="checkbox" <?php if ( $instance['teaser-thumb'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'teaser-thumb' ); ?>" name="<?php echo $this->get_field_name( 'teaser-thumb' ); ?>" />
				...
				<select id="<?php echo $this->get_field_id( 'teaser-thumb-placement' ); ?>" name="<?php echo $this->get_field_name( 'teaser-thumb-placement' ); ?>">
					<option value="left" <?php if ( 'left' == $instance['teaser-thumb-placement'] )
						echo 'selected="selected"'; ?>>Left
					</option>
					<option value="right" <?php if ( 'right' == $instance['teaser-thumb-placement'] )
						echo 'selected="selected"'; ?>>Right
					</option>
				</select>
				...
				<input class="checkbox" type="checkbox" <?php if ( $instance['teaser-date'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'teaser-date' ); ?>" name="<?php echo $this->get_field_name( 'teaser-date' ); ?>" />
			...
			<p>
				<input class="checkbox" type="checkbox" <?php if ( $instance['view-all'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'view-all' ); ?>" name="<?php echo $this->get_field_name( 'view-all' ); ?>" />
		...
		<div style="float:left;width:210px">
			...
			<p>
				<input class="checkbox" type="checkbox" <?php if ( $instance['custom-colors'] == on )
					echo checked; ?> id="<?php echo $this->get_field_id( 'custom-colors' ); ?>" name="<?php echo $this->get_field_name( 'custom-colors' ); ?>" />
			...
			<p>
				<input class="colorwellvideo<?php echo $number; ?>" id="<?php echo $this->get_field_id( 'header-color' ); ?>" name="<?php echo $this->get_field_name( 'header-color' ); ?>" type="text" maxlength="7" size="7" value="<?php echo $instance['header-color']; ?>" /> Header Bar
				<br />
